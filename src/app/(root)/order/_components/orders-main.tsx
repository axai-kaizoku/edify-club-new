...
import { getOrderByID } from "@/server/api/orderActions"
import { useQuery } from "@tanstack/react-query"
import SkeletonLoader from "./skeleton-orders-main"
import { useRouter } from "next/navigation"

const OrdersMain = ({ id }: { id: string }) => {
  ...
  if (error) {
    return (
      <Container className="flex flex-col items-center justify-center h-screen text-center">
        <p className="text-red-500 font-gilroySemiBold text-lg">
    ...
    <>
        ...
        <section className="flex flex-col gap-3 mb-2">
          <div className="rounded-[14px] border border-[#DEDEDE] flex flex-col gap-7 px-3 py-7">
            <div className="text-[#344054] text-2xl font-gilroySemiBold text-center">
              Order ID: {order?.cart?.razorpay_order_id}
            </div>
            <div className="flex flex-col gap-1">
              <div className="flex gap-1 mx-auto items-center">
                ...
                {formatDate(order?.cart?.createdAt)}
                </span>
              </div>
              ...
              <div className="flex gap-2 items-center mx-auto">
                ...
                <span className="text-base text-[#12B76A] font-gilroySemiBold">
                  Estimated delivery: {formatDate(order?.cart?.deliveryStatus?.date[0])}
                </span>
              </div>
            </div>
          ...
          <div className="flex flex-col border border-[#DEDEDE] px-5 py-4 rounded-[14px] w-full">
            ...
            {order?.cart?.address?.address}, {order?.cart?.address?.city},{" "}
                  {order?.cart?.address?.state}, {order?.cart?.address?.country}
                  , {order?.cart?.address?.landmark},{" "}
                  {order?.cart?.address?.pinCode} <br/> Phone: (+91) {order?.cart?.address?.phone}
            </div>
          </div>
        </section>
      </Container>
      ...
      <div className="flex flex-col gap-5 mt-6">
        <div className="flex gap-4 items-center lg:mx-16 md:mx-8 sm:mx-4 max-sm:hidden">
          <OrderIcons.back_icon className="cursor-pointer" onClick={()=>{ router.back()}}/>
          <div className="font-gilroySemiBold text-black text-xl 2xl:text-2xl">
            Order Details
          </div>
        </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                {/* <button className="flex justify-center items-center gap-2 text-white text-xs 2xl:text-sm  px-3 py-2 2xl:px-4 2xl:py-2 rounded-[8px] bg-[#7161EF] font-gilroySemiBold flex-nowrap">
                  <div className="font-gilroySemiBold text-nowrap">
                    Track order
                  </div>
                  <OrderIcons.location_icon />
                </button> */}
            ...
            <div className="flex gap-4 items-center">
              <div>
                ...
                <span className="font-gilroySemiBold text-[15px] 2xl:text-base text-[#1D2939]">
                  {order?.cart?.createdAt? formatDate(order?.cart?.createdAt) : "-"}
                </span>
              </div>
              ...
              <div className="flex gap-2 items-center">
                ...
                <span className="text-base text-[#12B76A] font-gilroySemiBold">
                  Estimated delivery:{" "}
                  {order?.cart?.deliveryStatus?.date[0] ? formatDate(order?.cart?.deliveryStatus?.date[0]) : "-"}
                </span>
              </div>
            </div>

            <OrderProgressBar status="Order Confirmed" />
              ...
              <div className="flex-1 flex flex-col gap-9 2xl:gap-10">
                <div>
                  ...
                  <div className="2xl:text-[14px] text-[13px] text-[#667085] items-center flex gap-3 font-gilroyRegular">
                    <div>{order?.cart?.paymentMethod}</div>
                    <OrderIcons.visa_icon />
                  </div>
                </div>
              </div>
              <div className="flex-1 flex items-center 2xl:gap-5 gap-x-20 justify-end">
                <span className="text-[#667085] 2xl:text-xl text-lg font-gilroyMedium">
                  Total
                </span>
                <span className="text-[#1D2939] 2xl:text-xl text-lg font-gilroyBold">
                  ₹ {order?.cart?.totalPrice}
                </span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-[14px] border border-[#DEDEDE] flex flex-col 2xl:py-5 2xl:px-6 py-4 px-5">
              ...
              <div className="lg:w-[55%] xl:w-[60%] 2xl:w-[65%] 2xl:text-lg text-base font-gilroyMedium text-[#667085] flex flex-col gap-2">
                <div>
                  ...
                  {order?.cart?.address?.state}, {order?.cart?.address?.country}
                  , {order?.cart?.address?.landmark},{" "}
                  {order?.cart?.address?.pinCode}
                </div>
                <div>Phone: (+91) {order?.cart?.address?.phone}</div>
              </div>
            </div>
          </div>
        </section>
