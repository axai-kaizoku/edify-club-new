...
import { createDeviceReview } from "@/server/api/productActions"
...
//   ],
// }
export const ReviewsSecx = ({data}:{data:any}) => {
  const { overallRating, overallReviews } = data.reviewAggregates
    ...
    <section className="w-full h-full bg-[#F7F7F7] rounded-3xl flex flex-col justify-center items-center my-14">
      ...
      <div className="flex w-[90%] xs:w-[75%] sm:w-[70%] lg:w-[40%] h-full justify-between items-center my-4">
        ...
        <div className="flex flex-col gap-y-1 lg:gap-y-2 items-center">
          ...
          <span className="text-[#858585] font-gilroyMedium text-sm lg:text-base">{`(${overallReviews+50} Reviews)`}</span>
        </div>
      </div>
      <CreateReview>
        <span
...
export const CreateReview = ({
  ...
  const [formData, setFormData] = useState<{
    ...
    rating: string
    userId: string
  }>({ comment: "", deviceId: deviceId ?? "", rating: "0", userId: "67bddb0323c01877b296a14c" })
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData((prev)=>({...prev, comment: "", deviceId: deviceId ?? "", rating: "0" }));
      router.refresh()
    } catch (e: unknown) {
  ...
  useEffect(() => {
    setFormData((prev)=>({...prev, comment: "", deviceId: deviceId ?? "", rating: "" }));
